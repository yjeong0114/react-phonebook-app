...
function collectionChanges(query, events) {
    if (events === void 0) { events = ALL_EVENTS; }
    return fromCollectionRef(query, { includeMetadataChanges: true }).pipe(windowwise(), operators.map(function (_a) {
...
function collection(query) {
    return fromCollectionRef(query, { includeMetadataChanges: true }).pipe(operators.map(function (changes) { return changes.docs; }));
}
/**
...
function auditTrail(query, events) {
    return collectionChanges(query, events).pipe(operators.scan(function (current, action) { return tslib.__spreadArray(tslib.__spreadArray([], current), action); }, []));
}
/**

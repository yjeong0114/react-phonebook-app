...
function list(query, events) {
    ...
    return fromOnce(query).pipe(operators.switchMap(function (change) {
        ...
        if (!change.snapshot.exists()) {
            return rxjs.of(change.snapshot.val());
        }
...
function auditTrail(query, events) {
    var auditTrail$ = stateChanges(query, events).pipe(operators.scan(function (current, changes) { return tslib.__spreadArray(tslib.__spreadArray([], current), [changes]); }, []));
